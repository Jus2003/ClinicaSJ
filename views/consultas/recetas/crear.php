...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$error) {
    try {
        ...
        header("Location: index.php?action=consultas/recetas/detalle&id={$id_receta}&success=" . urlencode("Receta creada exitosamente"));
        exit;
...
// Obtener pacientes para el selector (solo si no viene de consulta/cita específica)
$pacientes = [];
if (!$consultaInfo && $_SESSION['role_id'] == 1) {
    $sqlPacientes = "SELECT id_usuario, CONCAT(nombre, ' ', apellido) as nombre_completo, cedula
                     FROM usuarios 
